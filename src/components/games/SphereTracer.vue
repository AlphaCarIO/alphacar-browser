...
export default {
  mounted() {
    ...
    self.height = document.body.clientHeight / 2;
    if (self.game == null) {
      self.game = new Phaser.Game(self.width, self.height, Phaser.AUTO, 
        self.$refs.screen, { create: self.create, update: self.update });
    }
  },
  methods: {
    create() {
    ...
    self.filter = new Phaser.Filter(self.game, null, fragmentSrc);
    self.filter.setResolution(self.width, self.height);

    self.sprite = self.game.add.sprite();
    ...
    update() {
        let self = this;
        self.filter.update(self.game.input.activePointer);
    },
    destroyed() {
        let self = this;
        self.game.destroy();
